...
export default function WeeklyReportPage() {
  ...
  async function onSubmit(event: FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(data.error ?? 'Cannot fetch weekly report')
      return
    }
  ...
  return (
    <main className="max-w-4xl mx-auto py-12 px-4">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 font-serif mb-2">Weekly Report</h1>
        <p className="text-slate-500">Analyze your performance over the selected week.</p>
      </div>

      <div className="glass-card p-8 mb-8">
        <form className="flex flex-col sm:flex-row items-end gap-4" onSubmit={onSubmit}>
          <div className="w-full sm:flex-1">
            <label htmlFor="date" className="block text-sm font-semibold text-slate-700 mb-2">Select Reference Date</label>
            <input
              id="date"
              ...
              required
            />
            <p className="text-xs text-slate-400 mt-1">The report will cover the week containing this date.</p>
          </div>
        ...
        <div className="space-y-6 animate-fade-in-up">
           <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-4 text-center">
             <p className="text-indigo-800 font-medium">
               Week Range: <span className="font-bold">{new Date(result.range?.start).toLocaleDateString()}</span> - <span className="font-bold">{new Date(result.range?.end).toLocaleDateString()}</span>
             </p>
           </div>
